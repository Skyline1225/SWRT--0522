...
class MainWindow(QMainWindow):
    ...
    def create_info_panel(self):
        ...
        rules_summary = QLabel('1. SWRT输入文档E列="否"时提取C列ID\n2. 在SWRT平台文档P列匹配ID\n3. Q列设为NA，P列内容复制到R列')
...
2. 识别SWRT平台文档中P列"适用范围"列的内容，如果包含上一步骤提取的ID号相同的内容，则把该行Q列的内容改为NA，并把P列的内容复制到R列中"""
    ...
    def display_changes(self):
        ...
        self.changes_preview.setColumnCount(7)

        headers = ['变更时间', '工作表', '行号', '变更类型', 'Q列原值', 'Q列新值', 'R列新值']
    ...
    def _add_changes_sheet(self, handler):
        ...
        headers = ['序号', '变更时间', '工作表', '行号', '变更类型', 'Q列原值', 'Q列新值', 'R列新值', '适用范围']
